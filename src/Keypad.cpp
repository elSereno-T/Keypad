...
// 		if (key[i].keyState==IDLE) {
// 			key[i].mainChar = NO_KEY;
// 			key[i].kcode = -1;
...
// 			char keyChar = keymap[r * sizeKpd.cols + c];
// 			int keyCode = r * sizeKpd.cols + c;
...
// 			if ((idx == -1) && button) {
// 				for (byte i=0; i<LIST_MAX; i++) {
// 					if (key[i].mainChar==NO_KEY) {		// Find an empty slot or don't add key to list.
// 						key[i].mainChar = keyChar;
// 						key[i].kcode = keyCode;
// 						key[i].keyState = IDLE;		// Keys NOT on the list have an initial state of IDLE.
...
// 			else if (millis() - key[idx].start > key[idx].wait){
// 				transitionTo (idx, HOLD);
// 				key[idx].start += key[idx].wait;
// 				key[idx].wait = max(100, key[idx].wait-50);
// 			}
// 			break;
// 		case RELEASED:
// 			transitionTo (idx, IDLE);
...
// 	for (byte i=0; i<LIST_MAX; i++) {
// 		if ( key[i].mainChar == keyChar ) {
// 			if ( (key[i].keyState == PRESSED) && key[i].stateChanged )
// 				return true;
// 		}
// 	}
...
// 	key[idx].keyState = nextState;
// 	key[idx].stateChanged = true;
// 	if (nextState == PRESSED) {
// 		key[idx].start = millis();
// 	}
...
// 			keypadEventListener(key[0].mainChar);
// 		}
// 	}
...
// 			keypadEventListener(key[idx].mainChar);
// 		}
// 	}
// }
